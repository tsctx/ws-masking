...
async function initialize() {
  ...
  function _mask(source, mask, output, offset, length) {
    if (length <= memorySize) {
      ...
      while (fixedOffset + memorySize < length) {
        output.set(
          wasmMask(
            source.subarray(fixedOffset, fixedOffset + memorySize),
            mask,
            memorySize,
          ),
          fixedOffset,
        );
        fixedOffset += memorySize;
      }
      if (fixedOffset !== length) {
        output.set(
          wasmMask(
            source.subarray(fixedOffset, length),
            mask,
            length - fixedOffset,
          ),
          fixedOffset,
...
module.exports = {
  ...
  js: {
    /**
     * @type {(source: Uint8Array, mask: Uint8Array, output: Uint8Array, offset: number, length: number) => Uint8Array}
     */
    mask: js.mask,
    /**
     * @type {(buffer: Uint8Array, mask: Uint8Array) => Uint8Array}
     */
    unmask: js.unmask,
  },
};
